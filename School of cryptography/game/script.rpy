...
screen rar:
    ...
    frame:
        ...
        vbox:
            ...
            textbutton "Шифр Виженера" action Show ("vigener")
...
screen vigener:
    frame:
        ...
        yalign 0.5
        vbox:
            text "Шифр Виженера можно назвать более сложной версией шифра Цезаря. В шифре Цезаря каждая буква алфавита смещается на несколько позиций (подробнее в разделе «Шифр Цезаря). Шифр Виженера представляет из себя таблицу с различными значениями сдвига. Для зашифровки используют квадрат (таблицу) Виженера, которая состоит из строк и столбцов, по количеству равных символам в алфавите, каждая строка является шифром Цезаря с различным сдвигом. Во время шифрования используются разные шифры Цезаря, которые выбираются, исходя из символов ключевого слова. "
            text "Попробуем зашифровать «зеленое яблоко», используя ключ «шифр». Для этого запишем ключ под исходной фразой, повторяя его, пока фраза не закончится."
            text "ЗЕЛЁНОЕ ЯБЛОКО"
            text "ШИФРШИФ РШИФРШ"
            text "Буква «з» будет соответствовать букве «ш», значит, нужно найти в таблице пересечение столбца «з» и строки «ш» — это и будет зашифрованной буквой. Таким образом мы получим:"
            text " бобцжшъ ръхдьз"
            text "Для расшифровки мы точно так же пишем под зашифрованным текстом ключ, сопоставляем буквы фразы и ключа, ищем в строке буквы ключа букву зашифрованной фразы и смотрим, к какому столбцу она принадлежит, эта буква будет буквой исходной фразы."
...
label visener:
    ...
    scene classroom
    show happy
    e "Ты спраивлся с еще одной главой!"
    hide happy
    show normal
    e "Вот тебе еще одна милашная картинка, теперь с лягушкой."
    show milo
    e "Правда прелесть?"
    hide milo
    e "А теперь займемся нашей последней главой."
    e "В ней мы проходим шифр Виженера"
    show clever
    e "Этот шифр можно назвать более сложной версией шифра Цезаря."
    e "Шифр Виженера - это таблица с шифрами Цезаря с разным сдвигом."
    e "Для зашифровки используют квадрат (таблицу) Виженера, которая состоит из строк и столбцов, по количеству равных символам в алфавите, каждая строка является шифром Цезаря с различным сдвигом"
    e "Во время шифрования используются разные шифры Цезаря, которые выбираются, исходя из символов ключевого слова. "
    hide clever
    e "Попробуем зашифровать «зеленое яблоко», используя ключ «шифр». Для этого запишем ключ под исходной фразой, повторяя его, пока фраза не закончится."
    hide normal
    show ugol:
         xalign 0.001
         yalign 0.2
    ...
    show ugol:
         xalign 0.001
         yalign 0.2
    e "Даоьше нужно сопоставить каждую букву исходной фразы с буквой слова-ключа"
    e "После этого находим пересечение строки буквы ключа и столбца зашифрованной буквы"
    scene vig2
    show ugol:
         xalign 0.001
         yalign 0.2
    e "Буквы З и Ш дают на пересечении букву Б"
    e "Таким образом мы получим: бобцжшъ ръхдьз"
    scene classroom
    hide ugol
    show normal
    e "Чуть сложнее, чем предыдущие шифры."
    show happy
    e "Надеюсь твои мозги не вскипят, а то мне бы не хотелось жить с еще одним призраком."
    hide happy
    e "А теперь перейдем к заданию."
    e "Я нашлел книгу «Программирование для чайников», но чтобы ее открыть нужно ввести пароль."
    e "Я дам тебе слово-ключ, а ты должен зашифровать другое слово, чтобы получить пароль."
    e "Можешь потом ее прочитать, если захочешь."
    e "Ладно, мне пора спешить по моим загробным делам"
    show ugol
    e "Еслич что-то не понял, то справочник там, удачи!"
    hide ugol
    scene vigener1
    "Заблокированная книга"
    $ answer = renpy.input("Введите пароль")
    while answer != "ыауоаешьнэв":
        $ answer = renpy.input("Пароль неверный")
    "Пароль верный"
    scene classroom
    show normal
    e "Как вижу, ты уже справился с этой задачей."
    e "Оу, текст внутри зашифрован"
    show happy
    e "Хорошо, что я смог раздобыть ключ!"
    hide happy
    e "Ладно, сиди тут и не скучай."
    e "Может, там написано что-то умное"
    hide normal
    scene vigener2
    ""
    $ answer = renpy.input("Что же там написано?")
    while answer != "Аккуратный программист - быстрый программист":
        $ answer = renpy.input("Кажется, что-то другое")
